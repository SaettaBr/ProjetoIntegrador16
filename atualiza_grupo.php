<?php
		 ...
		 if (pg_num_rows($result0) != 0) {
		  if ($id != "") {
				if ($nome != "") {			   
			       if ($projeto != "") {
			   	   		if ($integ01 != "") {
							$sql1="update grupo SET id = '$id', nome = '$nome', num_proj = '$projeto' where id='$id'";
				   			pg_query($conexao, $sql1);
							$sql7="DELETE FROM participa where id_grupo='$id'";
							pg_query($conexao,$sql7);
	...
	/*								}		
						...
						echo "<script>window.location='list_grupo.php';alert('Aluno não informado !')</script>";
						}
						echo "<script>window.location='list_grupo.php';alert('Cadastro realizado com sucesso !')</script>";
					 }else{
				  echo "<script>window.location='list_grupo.php';alert('O projeto não foi informado.')</script>";
				  }
			 }else{
				echo "<script>window.location='list_grupo.php';alert('O nome não foi informado.')</script>";
			  }
		}else{
		 echo "<script>window.location='list_grupo.php';alert(O ID não foi informada.')</script>";
		  }
	}else{
		echo "<script>window.location='list_grupo.php';alert('Já existe um grupo cadastrado com esse ID.')</script>";
	 }
 }else{
  echo "<script>window.location='list_grupo.php';alert('Falha na conexão com o banco de dados.')</script>";
 }
*/
}
}
